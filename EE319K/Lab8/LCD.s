...
;6) If bit 4 is high, loop back to step 5 (wait for BUSY bit to be low)
		LDR R1, =SSI0_SR_R ;We check if busy
Busy	LDR R2, [R1]
		ANDS R2, R2, #0x10
		BNE Busy
		LDR R1, =0x400043FC ;We write command to D/C=PA6
		LDR R2, [R1]
		BIC R2, #0x40
		STR R2, [R1]
		LDR R1, =SSI0_DR_R ;We write the command
		STR R0, [R1]
		LDR R1, =SSI0_SR_R ;We check if busy
Ready	LDR R2, [R1]
		ANDS R2, R2, #0x10
...
TNF		LDR R2, [R1]
		ANDS R2, R2, #0x02
		BEQ TNF
		LDR R1, =0x400043FC ;We write DATA to D/C=PA6
		LDR R2, [R1]
		ORR R2, #0x40
		STR R2, [R1]
		LDR R1, =SSI0_DR_R ;We write the command
